...
<template>
  <div :id="domId" class="echarts-drown-map"></div>
</template>

<script>
  import echarts from 'echarts/lib/echarts';
  import chinaJson from '../assets/map/china.js'
  import mapping from '../assets/map/mapping.js'
  export default {
    name: 'echarts-drown-map',
    components:{},
    props: {
      mapConfig: {
        type: Object,
    ...
    watch: {
      myChart: function(){
        // 绑定下钻事件
        const eventType = this.mapConfig.drownType || 'click';
    ...
    mounted(){
      ...
      if( typeof this.mapConfig.drownType !== 'string'){
        throw "the mapConfig.drownType must be string";
        return;
      }
    ...
    methods: {
      ...
      registerMap(mapName, drillDown){
        ...
        if (mapName === 'china') {
          import('../assets/map/china.js').then( res => {
          ...
          if (mapping[mapName]) {
            ...
            import(`../assets/map/other/${mapGeoJson}.js`).then( res => {
              echarts.registerMap(mapName, res.default);
              this.myChart.setOption(option);
